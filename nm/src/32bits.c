...
/*   Updated: 2024/11/26 18:40:47 by ydumaine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int compare_symbols32(Elf32_Sym *sym1, Elf32_Sym* sym2, char *sym_str_tab, size_t sym_str_tab_size)
{
...
const char *find_section_name32(Elf32_Shdr *section_header_tab, const char *shstrtab_section, Elf32_Half index, Elf32_Half shnum)
{
    if (index >= shnum)
        return (NULL);
...
void sort_sym_tab32(Elf32_Sym *symtab, Elf32_Off symtab_size,char *sym_str_tab, size_t sym_str_tab_size)
{
    Elf32_Off i = 1;
    Elf32_Off j = 0;
    Elf32_Sym symb;
    ...
    while (i < symtab_size)
    {
        ...
        while (j > 0 && compare_symbols32(&symb,  &symtab[j - 1], sym_str_tab, sym_str_tab_size) < 0)
        {
            symtab[j] = symtab[j - 1];
            j--;
        }
...
Elf32_Shdr *find_section_header_by_name32(const char *section_name, Elf32_Shdr *section_headers_start_adress, char *shstrtab_section, Elf32_Half shnum)
{
...
Elf32_Shdr *find_section_header_by_type32(Elf32_Word flag, Elf32_Shdr *section_header_tab, int section_count) {
...
Elf32_Shdr *find_section_header_by_index32(Elf32_Half index, Elf32_Shdr *section_header_tab, Elf32_Half section_count) {
...
char determine_symbol_type32(const Elf32_Sym *sym, Elf32_Shdr *section_header_tab, const char *shstrtab_section, const char *symb_name, Elf32_Half shnum)
{
    ...
    if (strcmp(symb_name, "__bss_start") == 0)
        ft_printf("");
    ...
    if (sym->st_shndx != 0xFFF1)
    {
        if (sym->st_shndx > shnum)
            return 'U';
        const char * name = find_section_name32(section_header_tab, shstrtab_section, sym->st_shndx, shnum);
        if (!strcmp(".rodata", name))
            ...
            Elf32_Shdr *section = find_section_header_by_index32(sym->st_shndx, section_header_tab, shnum); 
...
void display_symbol_info32(const Elf32_Sym *symb_tab, size_t sym_tab_size, const char *sym_str_tab, Elf32_Shdr *section_header_tab, const char *shstrtab_section, Elf32_Half shnum, size_t sym_str_tab_size)
{
    Elf32_Off addr; 
    ...
    for (size_t i = 0; i < (sym_tab_size / sizeof(Elf32_Sym)); i++) 
    {
        ...
        {
            uint64_t st_value = symb_tab[i].st_value;
            if (st_value == 0)
                continue;
        }
        ...
        if (symb_tab[i].st_name != 0)
        {
            const char *symb_addr = sym_str_tab+ (Elf32_Off)symb_tab[i].st_name;
...
void parse_32_bits(Elf32_Ehdr *ehdr, MappedFile mapped_file, char *file_name, int print_file_name)
{
    ...
    Elf32_Shdr *sym_tab_section_header = find_section_header_by_type32(SHT_SYMTAB ,section_header_tab, ehdr->e_shnum);
    ...
    Elf32_Shdr *str_tab_header = &section_header_tab[sym_tab_section_header->sh_link];
    if (!str_tab_header)
    {
        ft_dprintf("ft_nm: %s: no symbols\n", file_name);
        munmap(mapped_file.mapped, mapped_file.size);
    ...
    Elf32_Off symb_str_offset  = str_tab_header->sh_offset;
    ...
    sort_sym_tab32(symtab_copy, sym_tab_size / sizeof(Elf32_Sym), sym_str_tab, str_tab_header->sh_size);
    if (print_file_name == 1)
        ft_printf("\n%s:\n", file_name);
    display_symbol_info32(symtab_copy, sym_tab_size, sym_str_tab, section_header_tab, section_shstrtab, ehdr->e_shnum, str_tab_header->sh_size);
    free(symtab_copy);
    munmap(mapped_file.mapped, mapped_file.size);
}